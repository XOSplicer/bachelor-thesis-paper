...
\begin{itemize}
    ...
    \item \textbf{Running Gaussian average} For each pixel in the video image $ I $ a running average is calculated, that is updated in each frame by the formula $ \mu{}_t = \alpha{}I_t + (1 - \alpha)\mu{}_{t-i} $, where $ \alpha $ sii 
    
    %\item \textbf{Mean filtering}
    %\item \textbf{Temporal median filter}
