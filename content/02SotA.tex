...
For a \ac{2D} image $ F \in \mathbb{N}^{X \times Y} $ the convolution $ H \in \mathbb{N}^{X \times Y} $ using the kernel $ K \in \mathbb{N}^{M_i \times M_j} $ is calculated with the formula
$ H_{x,y} = \sum_{i=0}^{M-1} \sum_{j=0}^{N-1} F_{x+1-a_i, x+j-a_j}K_{i,j} $ \autocite[][]{opencv2018kernel}.

Convolution kernels can be used to apply local 
