...
A discrete (digital) two dimensional \ac{RGB} color image can be represented as a matrix  $ M \in \mathbb{N}^{X \times Y \times 3} $.
The value of a pixel can then be obtained using the function $ f_M: \mathbb{N}^2 \rightarrow \mathbb{N}^3 ; (x, y) \mapsto f(x,y) = M_{x,y} $.
...
It is a small matrix with the same dimensions as the image it is applied to. For \ac{2D} images $ 3 \times 3 $ or $5 \times 5 $ kernels are used. 
