...
They could consider to offer an elevator system with an integrated visual system to customers with the needs described above, who construct a new building or want to upgrade an existing lift, as an optional feature at an additional charge.
...
Since elevator technology expert and computer vision expert need to work together for the development of an advanced visual solution in elevators, 
the costs in this phase are characterized by expenses for said experts, as well as laboratory equipment.
Developing resilient and high quality software requires careful planing and implementation and is therefore crucial for the costs at this stage.
...
The costs for the hardware includes multiple cameras, do neither need a extraordinary high resolution nor frame rate, and a general purpose computer to process the images, which needs to have enough computing power to process the images in near real-time. 
Furthermore an connection to the elevator controller is necessary which might require some kind of adapter.
...
For example in an production plant, reduced transport times for the produced goods could lead to reduced production costs in general.
...
In addition the kind of processing of the video data should be limited to the extend necessary for the purpose of the video system. 
