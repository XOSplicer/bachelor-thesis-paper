...
In addition only 
For example it is not neccessary to perform face detection, or to identify individuals.
...
(check) purpose limitation -> faster lift service
(check) data transparency -> must have a sign in / infront of the elevator
...
(check)confidentiality -> measures must be taken, so that video footage is not accessible from unauthorized parties. can operate offline and local
...
(check)- need to inform people of beeing filmed
(check)- missuse oppertunities: system must not be online, but if accessible from outside the video feed is a target of threats
(check)- EN 62676-4 category "Detektieren (Erfassung)" -> no need for high resulotion images
