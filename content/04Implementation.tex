...
\begin{itemize}
    ...
    \item Sequential control shows a higher mean waiting time then the other two (\texttt{+} \%, \texttt{+} 1568\%). However a significantly lower ride time (\texttt{-}76.41 \%, \texttt{-}71.09 \%), which is is only liner dependent on the travel distance.
\end{itemize}

- adaptive control delivers more cargo items than sequential (\texttt{+}40.13 perc avg) and collective control (\texttt{+}35.52 perc avg)
- collective (\texttt{+}58.27 perc avg) and adaptive control (\texttt{+}61.53 perc avg) deliver more passengers than sequential
- sequential control has high waiting time and low, near constant ride times compared to the other to
- adaptive control shows slightly higher wait time (\texttt{+}30.56 perc avg), but lower (\texttt{-}18.40 perc avg) and less spread ride times than collective control
