...
- video capture works, but has small viewving angle, so that corners are not covered, maybe other angle is better
- adjustable blurring helped to find better background subtractor results
- background substraction is not as good as i though it would be, often a body is not compleatly detected, only its outline is detected <- room, for improvement by maybe applying other image segmentation techniques combined with it, or maybe using a lower resolution
- volume reconstruction in general feasible, however its not compleatly correct but yields at least proportional results
- due do inconsistent foreground masks, many "holes" occure
