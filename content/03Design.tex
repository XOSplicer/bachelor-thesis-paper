...
TODO reading of camera images from all cameras, blurring them to counter noise
...
and each encountered voxel, that is labeled as 1 in the original image and has not yet been labeled by the connected component algorithm, it assigned a new unique label.
Furthermore all neighbouring voxels, which are labeled as 1 in the original image, are also labeled with this label.
When a voxel is encountered, which is al ready labeled, this label is also propagated to its neighbours that are labeled as 1 in the original image. 
The basics of this approach work exactly like in two dimensions, however the definition of neighbouring voxels is altered to match three
...
$ \{0, 1\}^{X \times Y \times Z} \rightarrow \mathbb{N}^{X \times Y \times Z} $
