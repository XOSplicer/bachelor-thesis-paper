...
Furthermore all neighbouring voxels, which are labeled as 1 in $ F_{vol} $, are also labeled with this label.
When a voxel is encountered, which is al ready labeled, this label is also propagated to its neighbours that are labeled as 1 in $ F_{vol} $. 
The basics of this approach work exactly like in two dimensions, however the definition of neighbouring voxels is altered to match three dimensions. 
In general all 26 voxels in the cube around a voxel are considered neighbours.
