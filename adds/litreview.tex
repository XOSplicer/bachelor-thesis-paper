...
Patent by Otis describing an elevator system compromising a passenger tracking system that uses multiple 2D and 3D sensors to capture depth data within the cabin and in the lobbies. The system tracks each objects and its various measurements and detects objects as well as passengers. The passengers are tracked along their trip to their destination to generate a traffic list.
It not only patents the system but also the method of doing so.
\emph{Highly relevant, since it describes exactly what could be done in a broader approach.}
...
\parstartcite{hakonen2003simulation}
